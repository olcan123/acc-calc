<template>
  <form class="w-full min-h-screen mx-auto bg-white dark:bg-gray-900 space-y-6 px-4 py-6" @submit.prevent="onSubmit">
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Banka Hesabını Güncelle</h1>
      <button
        type="submit"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <FieldSelect fieldName="bankId" labelName="Banka" placeholderName="Banka seçiniz" :options="getSelectBanks" />
...
const { id, bankAccountId } = useRoute().params;
...
await bankAccountStore.fetchBankAccount(bankAccountId);
...
const onSubmit = handleSubmit(async (formValues) => {
  ...
  await bankAccountStore.updateBankAccount(formValues);
});
</script>
