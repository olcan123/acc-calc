<template>
  ...
  <!-- DataTable -->
  <DataTable :value="bankAccounts" dataKey="id" responsiveLayout="scroll">
    ...
    <Column header="İşlemler" style="width: 180px">
      <template #body="{ data }">
        <div class="flex gap-2">
          <button
            @click="
              router.push({
                ...
                params: { partnerId, bankAccountId: data.bankAccountId },
              })
            "
            type="button"
          ...
          <button
            @click="confirmDelete({ partnerId, bankAccountId: data.bankAccountId })"
            type="button"
...
const confirmDelete = ({ partnerId, bankAccountId }) => {
  confirm.require({
    ...
    rejectLabel: "Hayır",
    accept: async () => {
      await bankAccountStore.deleteBankAccount(partnerId, bankAccountId);
    },
  });
};

await bankStore.fetchBanks();
await bankAccountStore.fetchBankAccounts(partnerId);
</script>
