<template>
  ...
  <!-- DataTable -->
  <DataTable :value="unitOfMeasures" responsiveLayout="scroll" dataKey="id">
    <Column header="ID">
      <template #body="{ index }">
        {{ index + 1 }}
      </template>
    ...
    <Column field="abbreviation" header="Kısaltma" />
    ...
    <Column header="İşlemler" style="width: 180px">
      <template #body="slotProps">
        <div class="flex gap-2">
          <!-- Düzenle Butonu -->
          <button
            @click="updateUnitOfMeasure(slotProps.data.id)"
            type="button"
          ...
          <!-- Sil Butonu -->
          <button
            @click="confirmDeleteUnitOfMeasure(slotProps.data.id)"
            type="button"
